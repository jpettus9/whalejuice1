...
export default function Game(){
  const [balance,setBalance]=useState(1000); const [last,setLast]=useState(null);
  ...
  return (<div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
    <h1 className="text-3xl font-extrabold">Vortex Roulette</h1>
    <div className="mt-6 rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 to-cyan-400/10 p-6">
      <div className="h-48 rounded-2xl bg-white/5 border border-white/10 grid place-items-center text-white/50">[ Vortex Roulette Placeholder ]</div>
      <div className="mt-4 flex items-center justify-between text-sm"><div>Balance: <span className="font-semibold">${balance}</span></div><div className="text-white/60">{last?`Last: bet $${last.bet} → win $${last.win}`:'No plays yet'}</div></div>
