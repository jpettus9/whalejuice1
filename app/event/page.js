...
const QuickParlay = dynamic(()=>import('../../components/QuickParlay'),{ssr:false});
              ...
              {bets.map((b,i)=>(
                <div key={b.id} className="rounded-lg border border-white/10 bg-white/5 p-3">
                  ...
                  <div className="mt-2 flex items-center justify-between text-sm">
                    <div className="text-white/60">Cash Out</div>
                    <div className="font-semibold">${b.cash}</div>
                  </div>
                  <button onClick={()=>cash(i)} className="mt-2 w-full px-3 py-2 rounded-lg bg-cyan-500 text-black font-semibold flex items-center justify-center gap-2">
